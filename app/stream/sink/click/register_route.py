...
class RegisterRoute:
    ...
    def stream_end(data_stream):
        ...
        reports = data_stream.key_by(click_base.ReportsClickWithValueKeyBy()). \
            ...
            apply(ReportsRegisterRouteApply())
...
class ReportsRegisterRouteApply(WindowFunction):
    def apply(self, key, window, values, collector):
        ...
        try:
            reports_key, reports_data = RegisterRouteReports(values).to_dict()
            ...
            CurlToAnalysis('flow', '/click/register_route', 'PATCH', json=data).curl()


class RegisterRouteReports:
