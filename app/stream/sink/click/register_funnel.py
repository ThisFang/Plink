...
class ReportsRegisterFunnelApply(WindowFunction):
    def apply(self, key, window, values, collector):
        ...
        try:
            reports_key, reports_data = RegisterFunnelReports(values).to_dict()
            ...
            CurlToAnalysis('flow', '/click/register_funnel', 'PATCH', json=data).curl()


class RegisterFunnelReports:
