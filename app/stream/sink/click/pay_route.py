...
class PayRoute:
    ...
    def stream_end(data_stream):
        ...
        # 报表落地
        data_stream.key_by(click_base.ReportsClickKeyBy()). \
            time_window(milliseconds(5000)). \
            apply(ReportsPayRouteApply())
...
class ReportsPayRouteApply(WindowFunction):
    def apply(self, key, window, values, collector):
        ...
        try:
            reports_key, reports_data = PayRouteReports(values).to_dict()
            ...
            CurlToAnalysis('flow', '/click/pay_route', 'PATCH', json=data).curl()


class PayRouteReports:
