...
class ReportsAdviceApply(WindowFunction):
    def apply(self, key, window, values, collector):
        ...
        try:
            reports_key, reports_data = AdviceReports(values).to_dict()
            ...
            CurlToAnalysis('flow', '/click/advice', 'PATCH', json=data).curl()
...
class AdviceReports:
    ...
    def __key_dict(self):
        ...
        self.channel = self.details_list[0].get('medium')
