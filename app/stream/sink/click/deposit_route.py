...
class DepositRoute:
    ...
    def stream_end(data_stream):
        ...
        data_stream.key_by(click_base.ReportsClickKeyBy()). \
            ...
            apply(ReportsDepositRouteApply())
...
class ReportsDepositRouteApply(WindowFunction):
    def apply(self, key, window, values, collector):
        ...
        try:
            reports_key, reports_data = DepositRouteReports(values).to_dict()
            ...
            CurlToAnalysis('flow', '/click/deposit_route', 'PATCH', json=data).curl()


class DepositRouteReports:
