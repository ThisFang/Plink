...
class VipOpen:
    ...
    def stream_explode(stream):
        ...
        流逻辑，必须实现
        """
        return stream.flat_map(VipOpenSave())


class VipOpenSave(FlatMapFunction):
    def flatMap(self, stream, collector):
        topic, ip, data_list = stream
        data_list = json.loads(data_list)
        for item in data_list:
            try:
                vip_open_obj = VipOpenArgs(item)
                vip_open_obj = vip_open_obj.to_dict()
            except Exception as e:
                logger().error('{}, {}, {}'.format(topic, e, item))
            else:
                collector.collect(('vip_login_reports', json.dumps(vip_open_obj)))
...
class VipOpenArgs:
    ...
    def _explode_reports(self):
        ...
        self.enjoy_count_pv = 1
        self.enjoy_count_uv = number
