...
class Login:
    ...
    def stream_explode(stream):
        ...
        流逻辑，必须实现
        """
        return stream.flat_map(LoginSave())


class LoginSave(FlatMapFunction):
    def flatMap(self, stream, collector):
        topic, ip, data_list = stream
        data_list = json.loads(data_list)
        for item in data_list:
            try:
                login_obj = LoginArgs(item)
                login_obj = login_obj.to_dict()
            except Exception as e:
                logger().error('{}, {}, {}'.format(topic, e, item))
            else:
                collector.collect(('vip_login_reports', json.dumps(login_obj)))
...
class LoginArgs:
    ...
    def _explode_reports(self):
        ...
        is_vip = self.args.get('isvip')
        is_vip = int(is_vip)

        self.startup_count_pv = 0
        self.startup_count_uv = 0

        self.vip_count_pv = 0
        self.vip_count_uv = 0

        self.login_count_pv = 1
        self.login_count_uv = number

        self.vip_login_count_pv = 1 if is_vip else 0
        self.vip_login_count_uv = number if is_vip else 0
