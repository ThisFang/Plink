...
class Viplist:
    ...
    def stream_explode(stream):
        ...
        流逻辑，必须实现
        """
        return stream.flat_map(ViplistSave())


class ViplistSave(FlatMapFunction,):
    def flatMap(self, stream, collector):
        topic, ip, data_list = stream
        data_list = json.loads(data_list)
        for item in data_list:
            try:
                viplist_obj = ViplistArgs(item)
                viplist_obj = viplist_obj.to_dict()
            except Exception as e:
                logger().error('{}, {}, {}'.format(topic, e, item))
            else:
                collector.collect(('viplist', json.dumps(viplist_obj)))
...
class ViplistArgs:
    ...
    def _explode_reports(self):
        ...
        self.vip_count_pv = number
        self.vip_count_uv = number
